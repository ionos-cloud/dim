...
/**
 * This class signs DNS zones.
 *
 * It's a modified copypasta of {@link JCEDnsSecSigner}.
 * The difference is that the generation of RRSIG records is done in a parallelStream.
 */


public class JCEDnsSecParallelSigner implements DnsSecSigner
{
    ...
    public JCEDnsSecParallelSigner()
    {
        this.mKeyConverter = null;
        this.mVerboseSigning = false;
    }

    public JCEDnsSecParallelSigner(boolean verboseSigning)
    {
        this.mKeyConverter = null;
        this.mVerboseSigning = verboseSigning;
    }

    private class SignTask {
        public RRset rrset;
        public ThreadLocal<List<DnsKeyPair>> keypairs;

        public SignTask(RRset rrset, ThreadLocal<List<DnsKeyPair>> keypairs) {
            this.rrset = rrset;
            this.keypairs = keypairs;
        }
    }

    private List<DnsKeyPair> cloneKeyPairs(List<DnsKeyPair> keypairs) {
        return keypairs.stream().map(kp -> new DnsKeyPair(kp.getDNSKEYRecord(), kp.getPrivateKeyString())).collect(
                Collectors.toList());
    }

    private ThreadLocal<List<DnsKeyPair>> makeThreadLocalKeyPairs(List<DnsKeyPair> keyPairs) {
        if (keyPairs == null || keyPairs.isEmpty())
            return null;
        return new ThreadLocal<List<DnsKeyPair>>() {
            @Override
            protected List<DnsKeyPair> initialValue() {
                return cloneKeyPairs(keyPairs);
            }
        };
    }
     ...
     */
    public List<RRSIGRecord> signRRset(RRset rrset, ThreadLocal<List<DnsKeyPair>> keypairs, Date start,
                                       Date expire) throws IOException,
            GeneralSecurityException
    {
        if (rrset == null || keypairs == null || keypairs.get() == null) return null;

        // default start to now, expire to start + 1 second.
        if (start == null) start = new Date();
        if (expire == null) expire = new Date(start.getTime() + 1000L);
        if (keypairs.get().size() == 0) return null;
        ...
        ArrayList<RRSIGRecord> sigs = new ArrayList<RRSIGRecord>(keypairs.get().size());

        // for each keypair, sign the RRset.
        for (DnsKeyPair pair : keypairs.get())
        {
    ...
    private Name addRRset(ArrayList<SignTask> signTasks, Name zonename, RRset rrset,
                          ThreadLocal<List<DnsKeyPair>> kskpairs, ThreadLocal<List<DnsKeyPair>> zskpairs, Date start,
                          ...
                          Name last_dname) throws IOException, GeneralSecurityException
    {
        ...
        if (rrset.getName().equals(zonename) && rrset.getType() == Type.DNSKEY)
        {
            ...
            if (kskpairs!= null)
            {
                signTasks.add(new SignTask(rrset, kskpairs));
        ...
        signTasks.add(new SignTask(rrset, zskpairs));
            ...
            GeneralSecurityException
    {
        ...
        ArrayList<SignTask> signTasks = new ArrayList<SignTask>();
        ArrayList<Record> signed_records = new ArrayList<Record>();
        Name last_cut = null;
        Name last_dname = null;

        ThreadLocal<List<DnsKeyPair>> zskpairs_tl = makeThreadLocalKeyPairs(zskpairs);
        ThreadLocal<List<DnsKeyPair>> kskpairs_tl = makeThreadLocalKeyPairs(kskpairs);
        ...
        for (ListIterator<Record> i = records.listIterator(); i.hasNext();)
        {
            ...
            last_cut = addRRset(signTasks, zonename, rrset, kskpairs_tl, zskpairs_tl, start,
        ...
        // add the last RR set
        addRRset(signTasks, zonename, rrset, kskpairs_tl, zskpairs_tl, start, expire,
                fullySignKeyset, last_cut, last_dname);


        signed_records.addAll(records);

        // do parallel signing
        List<List<RRSIGRecord>> rrsigs = signTasks.parallelStream().map(st -> {
            try {
                return signRRset(st.rrset, st.keypairs, start, expire);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }).collect(Collectors.toList());
        rrsigs.stream().forEach(rr -> {signed_records.addAll(rr);});
